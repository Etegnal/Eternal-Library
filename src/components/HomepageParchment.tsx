...
export default function HomepageParchment({ latestArticles, featuredPoems }: HomepageParchmentProps) {
  ...
  return (
    <div className="relative z-30 max-w-6xl mx-auto px-4 sm:px-6 pb-24">
      ...
      <div className="relative rounded-3xl bg-[#FFFDF5] border-4 border-[#D8C7A5] shadow-2xl p-6 sm:p-12 space-y-8 overflow-hidden">
        
        {/* PARCHMENT DECORATIVE FILIGREE CORNER ORNAMENTS */}
        <div className="absolute top-3 left-3 w-8 h-8 border-t-2 border-l-2 border-[#B89F70] pointer-events-none rounded-tl-xl" />
        <div className="absolute top-3 right-3 w-8 h-8 border-t-2 border-r-2 border-[#B89F70] pointer-events-none rounded-tr-xl" />
        <div className="absolute bottom-3 left-3 w-8 h-8 border-b-2 border-l-2 border-[#B89F70] pointer-events-none rounded-bl-xl" />
        <div className="absolute bottom-3 right-3 w-8 h-8 border-b-2 border-r-2 border-[#B89F70] pointer-events-none rounded-br-xl" />

        {/* DOUBLE INNER VINTAGE BORDER */}
        <div className="absolute inset-2.5 border border-[#EADBBD] pointer-events-none rounded-2xl" />

        {/* 1. SEAMLESS TWO-WAY CATEGORY TOGGLE (YAZILAR / ŞİİRLER) */}
        <div className="relative z-10 flex flex-wrap items-center justify-center gap-3 border-b border-[#8B4513]/20 pb-6">
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-full font-serif font-bold text-xs sm:text-sm uppercase tracking-widest transition-all duration-300 flex items-center gap-2 border ${
              activeTab === 'all'
                ...
                : 'bg-amber-100/70 text-[#362215] border-amber-300 hover:bg-amber-200/80'
            }`}
          >
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between border-b-2 border-[#8B4513]/30 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-serif font-bold text-xl text-[#362215] uppercase tracking-wider">
                      YAZILAR
                    </h2>
                  </div>
                  <Link
                ...
                <div className="space-y-4">
                  {latestArticles.map((article, idx) => (
                    <article key={article.id} className="group flex gap-4 p-4 rounded-2xl bg-[#FFFDF9] hover:bg-white border border-[#E6D7BC] shadow-sm transition-all">
                      {article.coverImage ? (
                        ...
                        <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl bg-amber-100/70 flex items-center justify-center shrink-0 border border-amber-200">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {featuredPoems.map((poem, idx) => (
                    <div key={poem.id} className="group p-4 sm:p-4.5 rounded-2xl bg-[#FFFDF9] hover:bg-white border border-[#E6D7BC] shadow-sm transition-all space-y-2">
